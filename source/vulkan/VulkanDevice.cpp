...
{	
		...
		if (deviceExtensions.size() > 0)
		{
			for (const char* enabledExtension : deviceExtensions)
			{
				if (!extensionSupported(enabledExtension)) {
					errs( "Enabled device extension \"" + std::string(enabledExtension) + "\" is not present at device level\n");
				}
			}
	...
	*/
	VkResult VulkanDevice::createBuffer(VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryPropertyFlags, VkDeviceSize size, VkBuffer *buffer, VkDeviceMemory *memory, void *data)
	{
		...
		VkBufferCreateInfo bufferCreateInfo = Vulgine::initializers::bufferCreateInfo(usageFlags, size);
		bufferCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
		VK_CHECK_RESULT(vkCreateBuffer(logicalDevice, &bufferCreateInfo, nullptr, buffer));

		// Create the memory backing up the buffer handle
		VkMemoryRequirements memReqs;
		VkMemoryAllocateInfo memAlloc = Vulgine::initializers::memoryAllocateInfo();
		...
		if (data != nullptr)
		{
			...
			{
				VkMappedMemoryRange mappedRange = Vulgine::initializers::mappedMemoryRange();
	...
	*/
	...
	{
		VkCommandBufferAllocateInfo cmdBufAllocateInfo = Vulgine::initializers::commandBufferAllocateInfo(pool, level, 1);
		VkCommandBuffer cmdBuffer;
		VK_CHECK_RESULT(vkAllocateCommandBuffers(logicalDevice, &cmdBufAllocateInfo, &cmdBuffer));
		// If requested, also start recording for the new command buffer
		if (begin)
		{
			VkCommandBufferBeginInfo cmdBufInfo = Vulgine::initializers::commandBufferBeginInfo();
	...
	*/
	void VulkanDevice::flushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue, VkCommandPool pool, bool free)
	{
		...
		VkSubmitInfo submitInfo = Vulgine::initializers::submitInfo();
		submitInfo.commandBufferCount = 1;
		submitInfo.pCommandBuffers = &commandBuffer;
		// Create fence to ensure that the command buffer has finished executing
		VkFenceCreateInfo fenceInfo = Vulgine::initializers::fenceCreateInfo(VK_FLAGS_NONE);
		VkFence fence;
